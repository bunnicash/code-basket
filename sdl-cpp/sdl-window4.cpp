...
int main (int argc, char* argv[]) {
    ...
    while(gameloop) {
        ...
        while(SDL_PollEvent(&event)) {
            ...
            if(event.type == SDL_KEYDOWN) {  // pressing it down, https://wiki.libsdl.org/SDL_Keycode
                std::cout << "Keyboard DOWN...\n";
                if(event.key.keysym.sym == SDLK_0) {  // using keycode, virtual key
                    std::cout << "Keyboard 0...\n";
                } else {
                    std::cout << "Not keyboard 0\n";
                }
            }
        }
    }
